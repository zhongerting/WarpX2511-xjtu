...
MultiParticleContainer::MultiParticleContainer (AmrCore* amr_core)
{
    ...
    // 遍历所有粒子物种，根据物种类型创建相应的粒子容器实例
    for (int i = 0; i < nspecies; ++i) {
        // Physical类型：标准物理粒子容器，用于处理等离子体中的常规粒子
        if (species_types[i] == PCTypes::Physical) {
            allcontainers[i] = std::make_unique<PhysicalParticleContainer>(amr_core, i, species_names[i]);
        }
        // RigidInjected类型：刚性注入粒子容器，用于处理粒子束注入
        // 特点：粒子在z<z0时沿直线运动，z>z0时按常规粒子处理
        // 应用：等离子体尾波加速模拟中的粒子束注入，避免boosted frame中空间电荷场过早影响
 ...
 * @brief 多粒子容器的主演化函数
 * 
 * 该函数负责管理所有粒子物种在一个时间步内的演化过程，是WarpX中粒子模拟的核心调度器。
 * 主要功能包括：
 * 1. 重置电流和电荷密度场（如果不跳过沉积）
 * 2. 遍历所有粒子容器，调用各自的Evolve函数进行推进和沉积
 * 
 * @param fields 场寄存器，包含所有电磁场和电流密度场
 * @param lev 当前网格层级
 * @param current_fp_string 电流场标识符（如"current_fp"、"current_fp_nodal"等）
 * @param t 当前物理时间
 * @param dt 时间步长
 * @param subcycling_half 子循环半步类型（None/FirstHalf/SecondHalf）
 * @param skip_deposition 是否跳过电流沉积（用于某些特殊推进方案）
                                ...
                                ImplicitOptions const * implicit_options)
{
    ...
    // 遍历所有粒子容器（allcontainers），调用各自的Evolve函数
    // allcontainers包含所有物理粒子物种和激光粒子容器
    // 每个容器独立演化自己的粒子，并将电流密度沉积到共享的场中
    for (auto& pc : allcontainers) {
